...
This document describes the security properties of the MongrelDB Ruby client
and how to report vulnerabilities.

## Overview

The MongrelDB Ruby client is a pure-Ruby gem (no native extensions) that
talks to `mongreldb-server` over HTTP using the standard library
`net/http`. The client itself holds no encryption keys and stores no data
...
The MongrelDB Ruby client has no runtime dependencies beyond the Ruby
...
- the MongrelDB Ruby client version, Ruby version, and OS,
